...
async function iniciarPix() {
  ...
  document.getElementById('pix-key-display').textContent = CONFIG.pix_chave;

  if (CONFIG.mp_access_token) {
    // Gera preferência real no Mercado Pago
    try {
      const body = {
        transaction_amount: total,
        description: 'Pedido Pastel da Vila',
      ...
      if (data.point_of_interaction?.transaction_data) {
        ...
        // QR Code
        const qrImg = document.querySelector('#qr-code-area img');
        if (txData.qr_code_base64) {
          qrImg.src = 'data:image/png;base64,' + txData.qr_code_base64;
        }
        // Copia-cola
        if (txData.qr_code) {
          document.getElementById('pix-key-display').textContent = txData.qr_code;
        }
      }
    } catch (err) {
      console.error('Erro Mercado Pago:', err);
      // fallback: exibe chave estática
    }
  }
